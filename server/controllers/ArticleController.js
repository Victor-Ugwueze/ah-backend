...
class ArticleController {
      ...
      .then(() => {
        Article.findOne({
          ...
          const notify = {
            ...
            authenticatedUser,
            articleUrl: `${process.env.BASE_URL}/api/v1/articles/${
              createdArticle.slug
            }`,
            message: `${authenticatedUser.firstname} published new article`
          };
      ...
      .then((foundArticle) => {
        if (foundArticle) {
          ...
          foundArticle
            ...
            .then((tagArticle) => {
              foundArticle.setTags(tags).then(() => foundArticle
  ...
  static getSingleArticle(req, res, next) {
    ...
    Article.findOne({
      ...
      },
      {
        model: Reaction,
        as: 'reactions',
        attributes: ['id', 'userId', 'likes', 'dislike']
      }],
      attributes: [
        'id',
        'userId',
  ...
  static tagIdExist(collectedtags) {
    const allCollectedTags = collectedtags.split(',');
    const filteredtag = [];
    for (let i = 0; i < allCollectedTags.length; i += 1) {
      Tag.findOne({ where: { id: parseInt(allCollectedTags[i], 10) } })
        .then((tag) => {
          if (tag) {
            filteredtag.push(allCollectedTags[i]);
          }
        })
