...
class UserController {
  ...
  */
  static editUserProfile(request, response) {
    ...
    if (request.file) {
      imageUrl = request.file.secure_url;
    }
    if (!favouriteTags) {
      favouriteTags = [];
    }
    User.findOne({
      ...
      const userDetails = {
        firstname: (firstname) || fetchedUser.firstname,
        lastname: (lastname) || fetchedUser.lastname,
        bio: (bio) || fetchedUser.bio,
        favouriteTags: (favouriteTags) || fetchedUser.favouriteTags,
        imageUrl: (imageUrl) || fetchedUser.imageUrl
      };
