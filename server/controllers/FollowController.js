...
class FollowController {
  ...
  static listFollowing(request, response) {
    Follow.findAndCountAll({
      ...
      const followerArray = followers.rows.map(follower => follower.followinId);
      console.log(followerArray, request.params.id, 'hhere');
      ...
      }).then(users => response.status(201)
        .json({
          following: users.rows,
